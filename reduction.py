...
def median_combine_frames( list_filepath, ddir_input, ddir_output, median_filename, frame_type=None, n_exts=2 ):
    """
    Median combines frames into a master frame.
    """

    if os.path.isdir( ddir_output )==False:
        os.makedirs( ddir_output )
    ...
    for i in range( nframes ):
        filepath = os.path.join( ddir_input, frames[i] )
...
def subtract_dim_from_bright( dim_ifilepath, bri_ifilepath, ofilepath, frame_type='', n_exts=2 ):
    hdu_dim = fitsio.FITS( dim_ifilepath, 'r' )
    hdu_bri = fitsio.FITS( bri_ifilepath, 'r' )
    if os.path.isfile( ofilepath ):
        os.remove( ofilepath )
    hdu_out = fitsio.FITS( ofilepath, 'rw' )
    hdu_out.write( None, header=None )
    header = { 'OBJECT':'master_{0}'.format( frame_type ) }
    hdu_out.write( hdu_bri[1].read() - hdu_dim[1].read(), header=header )
    if n_exts==2:
        hdu_out.write( hdu_bri[2].read() - hdu_dim[2].read(), header=header )
    hdu_out.close()
    print '\nSaved master {0}:\n{1}'.format( frame_type, ofilepath )
    return None
...
def calibrate_raw_science( n_exts=1, raw_ddir='', cal_ddir='', mbias_filepath='', mflat_filepath='', raw_science_list_filepath='' ):
    ...
    for i in range( nframes ):
        ...
        for j in range( n_exts ):
            ...
            header_j = raw_hdu[j].read_header()
            cal_hdu.write( cal_arr_j, header=header_j )
